...
等待用户点击指定的3D对象（或指定列表或组中的任何对象，或者如果使用所有对象拼图，则点击场景中的任何对象） - 然后在“执行”插槽或“错过：如果用户点击其他内容或在后台，请执行“插槽”。
...
等待用户将鼠标悬停在指定的3D对象（或指定列表或组中的任何对象，或者如果使用了所有对象拼图的场景中的任何对象）之上- 然后在“over / out：do”中运行拼图插槽。
...
跟踪指定对象（或指定列表或组中的任何对象，或场景中的任何对象，如果使用所有对象拼图）的任何移动（位置，旋转和缩放）。如果对象开始移动，则在“start：do”插槽中运行拼图，在“移动：执行”插槽中继续运行拼图，直到对象停止移动，然后在“停止：执行”插槽中运行拼图。“delta”参数表示触发此拼图所需的位置，旋转或比例的任何坐标（x，y或z）的绝对变化。“period”参数表示在再次检查运动之前拼图等待的渲染帧数。
...
等待用户使用鼠标或触摸手势拖动指定的3D对象（或指定列表或组中的任何对象，或者如果使用了所有对象拼图，则在场景上的任何对象） - 然后生成用于使用的移动数据“拖动移动”，“拖动旋转”或“拖动比例”拼图。还捕获已启动和已停止拖动的事件。
...
根据拖动时拼图生成的拖动移动数据， 移动指定的3D对象（或指定列表或组中的所有对象，如果使用了所有对象拼图，则移动场景上的所有对象）。使用下拉菜单限制移动到特定轴或平面，或根本不限制。
...
根据拖动时拼图生成的拖动移动数据， 旋转指定的3D对象（或指定列表或组中的所有对象，或者如果使用所有对象拼图，则为场景中的所有对象）。使用下拉菜单将旋转限制为特定轴。“空间”下拉列表允许在“本地”和“父”坐标空间之间切换。
...
根据拖动时拼图生成的拖动移动数据， 缩放指定的3D对象（或指定列表或组中的所有对象，或者如果使用所有对象拼图，则为场景上的所有对象）。使用下拉菜单限制缩放到特定轴。
